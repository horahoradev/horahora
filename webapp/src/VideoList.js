...
function Video(props) {
  ...
  return (
      <div className={inline ? "h-24 w-96 relative inline-block m-1" : "px-2 h-44 w-44 m-1"}>
        ...
        <div className="rounded relative inline-block w-44">
          ...
          />
          {!inline && <Rate className={ "relative -mt-8 z-30 float-right"} allowHalf={true} disabled={true} value={video.Rating}></Rate> }
          </div>
        {/* TODO(ivan): deal with text truncation (hoping to have a multi-line text truncation,
                        which can't be done purely in css) */}
        <div className={inline ? "inline-block w-36 align-top h-44 inline-flex ml-2 justify-between  h-24 flex-col" : ""}>
          ...
          <Rate className={"z-30 w-full"} allowHalf={true} disabled={true} value={video.Rating}></Rate>
        </div>
        </Link>
      </div>
...
function VideoList(props) {
  ...
  if (videos) {
    elements = [
      videos.map((video, idx) => <Video inline={inline} key={idx} video={video}/>),
    ];
  }
  ...
  for (let i = 0; i < 10; i++) {
    elements.push(
      <div key={`_padding_${i}`} className={inline ? "w-64" : VIDEO_ELEMENT_WIDTH} />
    );
  }

  return (
    <div className={inline ? "flex-col my-4 rounded border p-4 bg-white flex flex-wrap min-h-screen" : "my-4 rounded border p-4 bg-white flex flex-wrap min-h-screen"}>
