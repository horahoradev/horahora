...
func (m *ArchiveRequestRepo) GetContentArchivalRequests(userID int64) ([]Archival, []Event,  error) {
	...
	sql := "SELECT Url, last_synced, backoff_factor, downloads.id, coalesce(archival_events.video_url, ''), coalesce(archival_events.parent_url, ''), coalesce(event_message, ''), coalesce(event_time, Now()) FROM user_download_subscriptions s " +
